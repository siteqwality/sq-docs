---
title: Traces
description: OpenTelemetry-compatible distributed tracing — span trees showing where a request spent its time across services.
...
- For "how many requests per second" — that's a [metric](/observability/metrics/overview/).
- For full payload bodies — those are [logs](/observability/logs/overview/) (you can correlate via `trace_id`).
...
Traces are billed per span ingested + retention. Sampling is your friend — see [sampling how-to](/observability/traces/how-to/sampling/).
