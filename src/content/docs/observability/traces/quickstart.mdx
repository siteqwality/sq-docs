...
```bash
SITEQWALITY_API_KEY=<your API key> node index.js
curl http://localhost:3000/api/anything
```

The `auto-instrumentations-node` package automatically traces Express, HTTP, fs, common DB drivers, and more, so you don't have to write spans by hand.
