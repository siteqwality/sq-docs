...
- For **discrete events** with rich context: what happened, who triggered it, what state the system was in.
...
```bash
curl -X POST https://logs.siteqwality.com/ingest \
...
- `X-Log-Source: <name>` sets the `source` field on every log in the request.
- `X-Log-Parser: <parser-id>` applies a specific [parser](/observability/logs/how-to/parsers/) to every log.
...
<CardGrid>
  ...
  <LinkCard title="Patterns and analytics" description="Cluster noisy lines into templates and read volume, sources and error rates." href="/observability/logs/how-to/patterns/" />
