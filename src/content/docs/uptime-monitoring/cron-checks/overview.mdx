...
If the ping doesn't arrive within `check_interval_seconds` of the previous one, the check flips to failure and notifications fire.
...
4. A scheduler runs every minute checking every cron job. If `now - last_received_at > check_interval_seconds + grace_period`, the check flips to failure and notifications fire.
5. The next successful ping flips status back to healthy.

A grace period is automatically applied so a slightly-delayed job (running 24h 5m instead of exactly 24h) doesn't immediately page. Today the grace period is fixed at one check interval — see [grace period notes](/uptime-monitoring/cron-checks/how-to/grace-period/).
