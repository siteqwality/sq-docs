...
| `check_regions` | string[] | `["aws-us-east-1"]` | Regions to run from. Subset of [supported regions](#supported-regions). Every value currently resolves to `aws-us-east-1`, see the note there. |
...
These four values are accepted by the API:
...
<Aside type="danger" title="Only aws-us-east-1 has a browser runner today">
...
See [regions](/uptime-monitoring/browser-checks/overview/#regions) for the full behavior, including how `min_healthy_regions` is capped. HTTP checks are unaffected and do run in all four.
</Aside>
