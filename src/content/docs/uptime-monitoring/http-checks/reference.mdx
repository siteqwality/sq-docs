...
| `check_regions` | string[] | `["aws-us-east-1"]` | Locations the check runs from. Omitted or empty falls back to the default. Only the default has a runner behind it. See [check regions](#check-regions). |
...
<Aside type="danger" title="Setting a location with no runner does not fail loudly">
Validation checks the value against the list above, not against whether anything is deployed there, so a check configured for `aws-eu-west-2` is created successfully and simply never produces a result from London.

The quorum counts only locations that returned a success, and a configured location that reports nothing counts as **not healthy** rather than absent. So `check_regions: ["aws-us-east-1", "aws-eu-west-2"]` with `min_healthy_regions: 2` creates a monitor that can never report healthy.

Additional check locations are on the roadmap. Until then, leave the field unset.
</Aside>
...
| Plan | Failure bodies readable for |
|---|---|
| Free | 5 days |
...
Do not read the table as a statement about when the stored object is destroyed.
Object deletion is handled separately by the storage lifecycle on a longer
schedule, and it is not the same number.
