...
<Aside type="caution">
There's no per-seat trial. Every invited user counts toward your plan's seat limit immediately on accepting, so revoke unused invites before inviting more.
</Aside>
...
3. Copy the value (`sq_live_...`) when shown. It's only displayed once, so store it in your secrets manager.
