...
How often a monitor's checks fire. Free plan minimum is 5 minutes (300 seconds); paid plans go down to 30 seconds. Browser checks have their own discrete intervals (`5m`, `10m`, `15m`, `30m`, `1h`).
...
A visitor who confirmed an email address to be notified when the status page publishes an incident or maintenance. Email only.
...
An account-scoped key, 96 hexadecimal characters, sent as a Bearer token. Create one under **Account → API Keys**. An **ingest** key can only send telemetry; a **full-access** key can also call the API.
