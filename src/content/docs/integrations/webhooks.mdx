...
description: Send a JSON POST to your own URL on every notification, for ServiceNow, custom dashboards, internal buses, anything.
...
A **webhook integration** is the universal escape hatch. If we don't have a native integration for the tool you use, the webhook integration POSTs incident JSON to a URL you control: wire it into ServiceNow, an internal Slack-replacement, your audit log, anything.

<Aside type="tip">
[PagerDuty](/integrations/pagerduty/) and [OpsGenie](/integrations/opsgenie/) are now native integrations, and they close their alerts automatically when a monitor recovers. Prefer those over a hand-rolled webhook bridge.
</Aside>
...
- Be idempotent, since the same event can be retried on receiver failure.
- Accept large bodies (incidents with many updates can be 10–50KB).

## Securing the receiver

The webhook URL is the auth, so anyone who knows it can POST. Treat the URL as a secret.
