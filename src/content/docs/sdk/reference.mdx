...
| `apiBase` | string | `https://api.siteqwality.com` | Override for the config-fetch API. |
| `ingestBase` | string | `https://rum.siteqwality.com` | Override for the RUM ingest endpoints. |
| `replayBase` | string | `https://replay.siteqwality.com` | Override for the replay segment endpoint. |

`init` is idempotent: a second call while an instance exists is a no-op. On init the SDK fetches its server-side config (session filters + privacy settings) from `{apiBase}/v1/rum/config/{applicationId}` and refreshes it every 5 minutes.
