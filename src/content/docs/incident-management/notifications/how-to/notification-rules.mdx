...
`position` is sort order within the urgency. Useful for the dashboard, since the firing logic just walks rules by their `delay_minutes`.
...
| 5 min | Escalation policy advances to level 2 and pages the next person. |
