...
| `auto_created` | `true` when a monitor state change opened the incident; `false` for manual ones. |
...
2. Incident opens automatically with `auto_created: true`, `status: investigating`, `responder_status: triggered`, `affected_http_job_ids: [<job_id>]`.
