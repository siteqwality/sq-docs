...
Find it under **Account → Audit Log**. Only account **owners and admins** can read it, because it exposes other members' actions and IP addresses.
...
<Aside type="caution" title="Owner or admin session required">
The endpoint is gated on the owner or admin role, which only a logged-in user session carries. API keys and M2M clients get a `403`: a leaked automation credential must not be able to read every member's actions and IP addresses. Programmatic export for SIEM ingestion is on the roadmap.
</Aside>
